...
#   Settings    (29pt @3x -> the 87px asset — the size the question is about)
#   Live Activity lock-screen card   (AppIconTile at 20pt)
#   Dynamic Island expanded          (AppIconGlyph at 19pt)
#   Dynamic Island compact           (AppIconGlyph at 18pt)
#   Dynamic Island minimal           (AppIconGlyph at 17pt)
#
...
OUTDIR="${OUTDIR:-/Users/albertxu/Documents/firstmate/data/wir-icon-adopt}"
...
# ============ 1: Home Screen ===============================================
echo; echo "=== 1/7: Home Screen ==="
xcrun simctl terminate "$SIM" "$BUNDLE_ID" >/dev/null 2>&1 || true
home; sleep 2
status_bar_override
shot "01-home-screen" ${HOME_CROP:-690 1660 250 250}

# ============ 2: Spotlight =================================================
echo; echo "=== 2/7: Spotlight ==="
home; sleep 1
drag 660 700 660 1500
sleep 2
typed "Gonna"
sleep 3
shot "02-spotlight" 0 380 1320 460

# ============ 3: Settings ==================================================
echo; echo "=== 3/7: Settings (29pt @3x — the 87px asset) ==="
home; sleep 1
xcrun simctl launch "$SIM" com.apple.Preferences >/dev/null 2>&1 || true
sleep 3
status_bar_override
for _ in 1 2 3 4 5 6; do drag 660 2200 660 700; sleep 0.4; done
sleep 2
shot "03-settings-list" 0 400 1320 2200

# ============ 4: Live Activity, Lock Screen card ===========================
echo; echo "=== 4/7: Live Activity lock-screen card ==="
for code in A B C; do
  ...
  shot "04-lockscreen-$code" 0 1500 1320 900
  devmenu "Lock"; sleep 2
  home; sleep 2
done

# ============ 5: Dynamic Island, expanded ==================================
echo; echo "=== 5/7: Dynamic Island expanded ==="
for code in A B C; do
  echo "--- scenario $code ---"
  home; sleep 1
  start_scenario "$code" || continue
  home; sleep 3
  status_bar_override
  press_shot "$ISLAND_X" "$ISLAND_Y" 2.0 "05-island-expanded-$code" 60 0 1200 620
done

# ============ 6: Dynamic Island, compact ===================================
echo; echo "=== 6/7: Dynamic Island compact ==="
for code in A B C; do
  echo "--- scenario $code ---"
  home; sleep 1
  start_scenario "$code" || continue
  ...
  status_bar_override
  sleep 2
  shot "06-island-compact-$code" 300 20 760 200
done
