...
#
# This opens Simulator.app and drives it — it is GUI capture work, so do not run
# it from an unattended session on someone's desk (see AGENTS.md, "Driving the
# iOS Simulator"). The screenshot itself is upright even though the Simulator
# window draws rotated; this script never taps, so the rotation trap does not
# apply here.
#
...
# iPhone 17 Pro is the simulator AGENTS.md standardises on for CI and local runs.
SIM_NAME="${SIM_NAME:-iPhone 17 Pro}"
# Gitignored: these captures are ~6 MB a run and are review artefacts, not
# history. Attach them to the PR instead.
...
Now check by eye — these are the assertions a script cannot make:
  ...
  A,  B,  C   cyan track, droplet glyph in the island
  AS, BS, CS  pale near-white track, snowflake glyph in the island
  BX          MUST be identical to B (cyan, droplet) with a TICKING countdown

The header badge on the card is the app icon on every scenario, rain or
wintry: it is the activity's identity, not its weather, and does not change.

On the wintry cards specifically (this is the W1b palette):
  * the track is near-white with a WHITE halo, not a blue one
  * the rail runs THROUGH the "now" dot and the hour dots, not 6pt below them
    (the 1.1 misalignment this release fixes, on rain cards too)
  ...
  * the app-icon badge in the header is unchanged from the rain cards
