...
- All four source folders (`WillItRain`, `WillItRainWidgets`, `Shared`, `WillItRainTests`) are file-system-synchronized groups in the pbxproj: a new `.swift` file dropped into one is compiled into that target with no project edit. Pure logic is tested by injecting the clock and stores (`PrecipitationPeriod.detect(in:)`, `RainForecast`'s `at:` readings, `NotificationSettings(defaults:)`, `NotificationService(deliver:)`); follow that pattern rather than reaching for `Date()` or `UserDefaults.standard` in new logic.
