...
- WeatherKit's minute forecast is regional. `ForecastMerge` (iOS) and `nextHour.ts` (Worker) fall back to the hourly forecast from the hour containing now when it is absent, and an hourly reading stands for the whole hour it starts (`ChartDataPoint.span`); `RainForecast.hasMinuteForecast` is what the UI keys the "showing hourly" line on. Keep both fallbacks in step. Inside minute coverage a period that begins on hourly data past the nowcast's end is `isConfirmed == false`: the hero line and charts show it, but `NotificationService` and `LiveActivityService` act only on `RainForecast.confirmedPeriods` (its onset is the moving nowcast horizon, so alerting on it re-announced one wet hour every poll), just as the Worker reads only `forecastNextHour` when it exists.
